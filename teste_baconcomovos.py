...
class TesteBaconComOvos(unittest.TestCase):
    def test_bacon_com_ovos_deve_levantar_assertionError_se_o_numero_n_for_inteiro(self):
    ...
    def test_bacon_com_ovos_deve_retornar_passar_fome_se_entrada_nao_for_multiplo_de_3_e_5(self):
        ...
        for entrada in entradas:
            with self.subTest(entrada=entrada, saida=saida):
                self.assertEqual(bacon_com_ovos(entrada), saida, msg=f'"{entrada}", não retorno "{saida}" ')
